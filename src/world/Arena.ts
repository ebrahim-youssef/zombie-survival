...
export class Arena {
  ...
  constrainPlayer(
    ...
    const normalizedDistance =
      Math.abs(dx) / safeHalfWidth +
      Math.abs(dy) / safeHalfHeight;
  ...
  private createWindows(): readonly ArenaWindow[] {
    ...
    if (!top || !right || !bottom || !left) {
      throw new Error(
        "Arena vertices were not initialized correctly.",
      );
    }
  ...
  private createWindow(
    ...
    const center = Phaser.Math.LinearXY(
      start,
      end,
      0.5,
    );
  ...
  private createInteractionLayout(): ArenaInteractionLayout {
    return {
      kudaWallBuy: new Phaser.Math.Vector2(
        this.center.x,
        this.center.y - this.halfHeight * 0.68,
      ),
  ...
  private splitWall(
    ...
    const firstEnd = Phaser.Math.LinearXY(
      start,
      end,
      0.5 - halfGap,
    );
    const secondStart = Phaser.Math.LinearXY(
      start,
      end,
      0.5 + halfGap,
    );
  ...
  private drawFloorGrid(): void {
    ...
    for (
      ...
      const a = Phaser.Math.LinearXY(
        this.vertices[3]!,
        this.vertices[0]!,
        t,
      );
      const b = Phaser.Math.LinearXY(
        this.vertices[2]!,
        this.vertices[1]!,
        t,
      );
